...
/* RCS Info: $Revision: 1.1 $ on $Date: 91/04/02 12:03:08 $
 *           $Source: //pepper/atesse_spice/spice3/HLP/RCS/provide.c,v $
    ...
    topic *top;
{
    ...
    if (hlp_usex) {
        if (!hlp_xdisplay(top)) {
            fprintf(stderr, "Couldn't open X window\n");
            return;
        }
    } else {
        if (!hlp_tdisplay(top)) {
...
#ifdef GI_X11       /* X11 does this asynchronously */
    if (hlp_usex) return;
#endif
