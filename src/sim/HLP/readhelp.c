...
/* RCS Info: $Revision: 1.1 $ on $Date: 91/04/02 12:03:09 $
 *           $Source: //pepper/atesse_spice/spice3/HLP/RCS/readhelp.c,v $
...
fplace *place;
{
    ...
    (void) fgets(buf, BSIZE, place->fp);    /* skip subject */
    (void) fgets(buf, BSIZE, place->fp);
    for (s = buf; *s && (*s != '\n'); s++)
        ;
    *s = '\0';
    ...
    /* get the text */
    /* skip to TEXT: */
    while (fgets(buf, BSIZE, place->fp)) {
    ...
    while (mof && !strncmp("TEXT: ", buf, 6)) {
        ...
        *s = '\0';
        ...
        if ((i = strlen(&buf[6]) - fchanges) > top->maxcols)
            top->maxcols = i;
        mof = fgets(buf, BSIZE, place->fp) == NULL ? false : true;
    }
endtext:
...
#ifdef notdef
/* *ss is of the form "subject subject ... " */
static toplink *getseetoplink(ss)
char **ss;
{
    toplink *tl;
    char *s;
    char subject[BSIZE], *t;

    if (!**ss) return(NULL);

    tl = alloc(toplink);
    for (s = *ss, t = subject; *s && *s != ' ' && *s != '\n'; s++) {
      *t++ = *s;
    }
    *t = '\0';
    tl->place = findglobalsubject(subject);
...
char *filename, *subject;
{
    ...
    sprintf(buf, "%s/%s.idx", hlp_directory, filename);
    hlp_pathfix(buf);
    if (!(fp = fopen(buf, "r"))) {
      perror(buf);
      return(-1);
