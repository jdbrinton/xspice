...
RCSID("INPevaluate.c $Revision: 1.1 $ on $Date: 91/04/02 11:56:43 $")
    ...
    int gobble; /* non-zero to gobble rest of token, zero to leave it alone */
{
    ...
    if( (*here == (char)NULL) || ((!(isdigit(*here))) && (*here != '.'))) {
        ...
        case 'M':
            {
                ...
                if(*(here+1) != (char) NULL && *(here+2) != (char) NULL) {
