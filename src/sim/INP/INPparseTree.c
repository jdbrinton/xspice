/* RCS Info: $Revision: 1.1 $ on $Date: 91/04/02 11:57:05 $
 *           $Source: //pepper/atesse_spice/spice3/INP/RCS/INPparseTree.c,v $
...
RCSID("INPparseTree.c $Revision: 1.1 $ on $Date: 91/04/02 11:57:05 $")
...
#define LOG_E   0.43429448190325182765
#define EE  2.71828182844590452353
#ifndef PI
...
/* printf("differentiating: "); printTree(p); printf(" wrt var %d\n", varnum);*/
    ...
    switch (p->type) {
        ...
        switch (p->funcnum) {
            ...
            arg1 = mkb(PT_DIVIDE, mkcon((double) LOG_E), p->left);
            break;
...
/* / */ { G, G, G, G, G, L, L, L, G, L },
/* ^ */ { G, G, G, G, G, L, L, L, G, L },
/* u-*/ { G, G, G, G, G, G, G, L, G, L },
/* ( */ { R, L, L, L, L, L, L, L, E, L },
/* ) */ { G, G, G, G, G, G, G, R, G, R },
/* v */ { G, G, G, G, G, G, G, G, G, R }
    ...
    while ((sp > 1) || (next->token != TOK_END)) {
        ...
        switch (prectable[top->token][next->token]) {
            ...
            if (st == sp) {
                ...
                if ((lpn == NULL) || (stack[sp].type !=
                        TYP_STRING))
                    goto err;
                if (!(pn = mkfnode(stack[sp].value.string,
                        lpn)))
                    return (NULL);
            } else { /* node op node */
