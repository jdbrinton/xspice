...
/* RCS Info: $Revision: 1.1 $ on $Date: 91/04/02 12:12:20 $
 *           $Source: //pepper/atesse_spice/spice3/FTE/RCS/sconvert.c,v $
    ...
    char **av;
{
    ...
    struct plot *pl;
    int i;
    char *infile = "/tmp/scinXXXXXX";
    char *outfile = "/tmp/scoutXXXXXX";
    FILE *fp;
    cp_in = stdin;
    cp_out = stdout;
    cp_err = stderr;
    ...
    switch (ac) {
        ...
        case 3:
            ...
            sf = infile = mktemp(infile);
            af = outfile = mktemp(outfile);
            if (!(fp = fopen(infile, "w"))) {
                perror(infile);
                exit(EXIT_BAD);
            }
            while (i = fread(buf, 1, BSIZE, stdin))
