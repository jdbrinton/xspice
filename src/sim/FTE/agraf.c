...
/* RCS Info: $Revision: 1.1 $ on $Date: 91/04/02 12:10:51 $
 *           $Source: //pepper/atesse_spice/spice3/FTE/RCS/agraf.c,v $
...
/* HPUX uses 3 digits for the exponent in %e format... This is a bad way to
    ...
    bool xlog, ylog, nointerp;
{
    ...
    /* Make sure we have the margin correct... */
    ...
    if (maxx <= 0) {
        fprintf(cp_err, "Warning: no points to plot...\n");
        return;
    }
...
#ifdef HPUX
    for (i = 1; (i < 11) && (xscale->v_name[i - 1]); i++)
        line2[i] = xscale->v_name[i - 1];
    if (!novalue)
        for (i = 13; (i < 22) && (vecs->v_name[i - 13]); i++)
            line2[i] = vecs->v_name[i - 13];
#else
    for (i = 1; (i < 10) && (xscale->v_name[i - 1]); i++)
        line2[i] = xscale->v_name[i - 1];
    if (!novalue)
        for (i = 12; (i < 20) && (vecs->v_name[i - 12]); i++)
            line2[i] = vecs->v_name[i - 12];
#endif
