...
/* RCS Info: $Revision: 1.1 $ on $Date: 91/04/02 12:11:23 $
 *           $Source: //pepper/atesse_spice/spice3/FTE/RCS/define.c,v $
...
{
    int arity = 0, i;
    char buf[BSIZE], tbuf[BSIZE], *s, *t, *b;
    wordlist *wl;
    struct pnode *pn;
    struct udfunc *udf;
...
ft_pnode(pn)
    int pn;
{
    prtree1((struct pnode *) pn, cp_err);
}
