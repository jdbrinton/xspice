...
/* RCS Info: $Revision: 1.1 $ on $Date: 91/04/02 12:11:16 $
 *           $Source: //pepper/atesse_spice/spice3/FTE/RCS/cpinterface.c,v $
...
#ifdef DIRCHAR
    for (s = cp_program; *s; s++)
        ;
    s--;
    while ((s > cp_program) && (*s != DIRCHAR))
        s--;
    if (*s == DIRCHAR)
        s++;
    (void) strcpy(buf, s);
    ...
    *s = '\0';
...
#ifdef LIBPATH
#ifdef VMS
    (void) sprintf(buf, "sourcepath = ( [] %s )", LIBPATH);
#endif
#ifdef UNIX
    (void) sprintf(buf, "sourcepath = ( . %s )", LIBPATH);
#endif
    ...
    for (s = cp_tildexpand(LIBPATH); *s; ) {
        ...
        if (fp = fopen(buf, "r")) {
            ...
            fprintf(cp_err, "Can't open %s...\n");
    }
    if (!found)
        fprintf(cp_err, 
    "Warning: can't find system init file - contact your spice guru\n");

#else
    fprintf(cp_err, 
        "Warning: LIBPATH undefined - contact your spice guru\n");
#endif
