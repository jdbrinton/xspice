...
/* RCS Info: $Revision: 1.2 $ on $Date: 92/05/04 17:56:38 $
 *           $Source: //pepper/atesse_spice/spice3/FTE/RCS/evaluate.c,v $
...
/* gtri - begin - Fix seg fault problem with 'plot v(<num>)' where <num> */
/*                doesn't exist. */

    if(!d)
        return(NULL);

/* gtri - end - Fix seg fault */
    ...
    struct pnode *arg1, *arg2;
{
    ...
    if (ind->v_length != 1) {
        fprintf(cp_err, "Error: strange range spec...\n");
        return (NULL);
    }
    if (isreal(ind)) {
...
#ifdef notdef /* ... I don't want to do this... */
    nscale = alloc(dvec);
    nscale->v_flags = scale->v_flags;
    nscale->v_name = mkcname('R', scale->v_name, ind->v_name);
    nscale->v_defcolor = scale->v_defcolor;
    nscale->v_gridtype = scale->v_gridtype;
    nscale->v_plottype = scale->v_plottype;
    nscale->v_type = scale->v_type;
    nscale->v_length = len;
    nscale->v_scale = nscale;
    nscale->v_numdims = scale->v_numdims;
    for (i = 0; i < scale->v_numdims; i++)
        nscale->v_dims[i] = scale->v_dims[i];

    if (isreal(nscale))
        nscale->v_realdata = (double *) tmalloc(sizeof (double) * len);
    else
        nscale->v_compdata = (complex *) tmalloc(sizeof (complex) *
                len);
#endif
...
#ifdef notdef
            if (isreal(nscale)) {
                nscale->v_realdata[j] = scale->v_realdata[i];
            } else {
                realpart(&nscale->v_compdata[j]) =
                    realpart(&scale->v_compdata[i]);
                imagpart(&nscale->v_compdata[j]) =
                    imagpart(&scale->v_compdata[i]);
            }
#endif
            j++;
        }
     ...
     * Doing this will cause a lot of core leaks, though...
     */
#ifdef notdef
    nscale->v_plot = scale->v_plot;
#endif
    ...
    struct pnode *arg1, *arg2;
{
    ...
    majsize = v->v_dims[v->v_numdims - 1];
    ...
    if (isreal(res))
        res->v_realdata = (double *) tmalloc(sizeof (double) *
                length);
    else
        res->v_compdata = (complex *) tmalloc(sizeof (complex) *
                length);
    ...
    struct pnode *arg;
{
     ...
     */
    if (!func->fu_func) {
        if (!arg->pn_value || (arg->pn_value->v_length != 1)) {
    ...
    for (; v; v = v->v_link2) {
        ...
        (void) signal(SIGILL, sig_matherr);
#endif

        if (eq(func->fu_name, "interpolate"))       /* Ack... */
    ...
    struct pnode *arg1, *arg2;
{
    ...
    /* This depends somewhat on what the operation is... Should fix... */
