...
RCSID("TFanal.c $Revision: 1.1 $ on $Date: 91/04/02 12:08:55 $")
...
#define outuid (uids[2]) /* unique id for the transfer function out. imp. */
    ...
    if(Itype != -1) {
        error = CKTfndDev((GENERIC*)ckt,&Itype,&ptr,
                ((TFan*)ckt->CKTcurJob)->TFinSrc, (GENERIC *)NULL,(IFuid)NULL);
        if(error ==0) {
        ...
        error = CKTfndDev((GENERIC*)ckt,&Vtype,&ptr,
                ((TFan*)ckt->CKTcurJob)->TFinSrc, (GENERIC *)NULL,
                (IFuid)NULL);
