...
MFBSetLineStyle(styleId)
    ...
    if(styleId == 0 && MFBFORMAT.setSolidLineStyle != NULL
        && *MFBFORMAT.setSolidLineStyle != NULL)
...
MFBSetFillPattern(styleId)
    ...
    if(styleId == 0 && MFBFORMAT.setSolidFillPattern != NULL
        && *MFBFORMAT.setSolidFillPattern != NULL)
...
MFBSetRubberBanding(RubberBandingBool,X,Y)
    ...
    || *MFBFORMAT.enableRubberBanding == NULL)
    return(MFBNORBND);
...
MFBDrawLineTo(X1,Y1)
    ...
    || *MFBFORMAT.drawLineToSequence == NULL){
...
MFBLine(X1,Y1,X2,Y2)
    ...
    && *MFBFORMAT.drawSolidLineSequence != NULL)
...
MFBBox(l,b,r,t)
    ...
    && *MFBFORMAT.drawBoxSequence != NULL){
...
MFBCircle(x,y,r,s)
    ...
    || *MFBFORMAT.drawCircleSequence == NULL){
    MFBCurrent->X = x + r;
    MFBCurrent->Y = y;
    if(s>2 && s<181)
        j = MFBmax(1,360/s);
