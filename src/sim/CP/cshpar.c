...
/* RCS Info: $Revision: 1.1 $ on $Date: 91/04/02 12:04:11 $
 *           $Source: //pepper/atesse_spice/spice3/CP/RCS/cshpar.c,v $
 ...
 * 
 * Now, in front.c these things get done:
    ...
    wordlist *wl;
{
    ...
    if (wl == NULL) {
        pw = getpwuid(getuid());
        if (pw == NULL) {
            fprintf(cp_err, "Welcome to the twilight zone...\n");
            return;
        }           
        s = pw->pw_dir;
    } else
...
com_shell(wl)
    wordlist *wl;
{
    fprintf(cp_err, "Error: command only available with UNIX or VMS.\n");
    return;
}
...
com_chdir(wl)
    wordlist *wl;
{
    fprintf(cp_err, "Error: command only available with UNIX.\n");
    return;
}
