...
/* RCS Info: $Revision: 1.1 $ on $Date: 91/04/02 12:04:17 $
 *           $Source: //pepper/atesse_spice/spice3/CP/RCS/history.c,v $
    ...
    wordlist *wlist;
{
    ...
    if (cp_lastone == NULL) {
        cp_lastone = histlist = alloc(histent);
    } else {
        cp_lastone->hi_next = alloc(histent);
