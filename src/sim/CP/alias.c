...
/* RCS Info: $Revision: 1.1 $ on $Date: 91/04/02 12:04:06 $
 *           $Source: //pepper/atesse_spice/spice3/CP/RCS/alias.c,v $
    ...
    wordlist *wlist;
{
    ...
    if (cp_aliases == NULL) {
        ...
        if (al->al_prev) {
            al = al->al_prev;
            ta = al->al_next;
            al->al_next = alloc(alias);
            al->al_next->al_prev = al;
            al = al->al_next;
            al->al_next = ta;
            ta->al_prev = al;
        } else {
            cp_aliases = alloc(alias);
