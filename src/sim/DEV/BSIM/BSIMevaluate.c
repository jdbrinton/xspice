...
RCSID("BSIMevaluate.c $Revision: 1.1 $ on $Date: 90/10/11 12:41:04 $")
...
BSIMevaluate(vds,vbs,vgs,here,model,gmPointer,gdsPointer,gmbsPointer,
...

 {
    ...
    K2   =  here->BSIMK2;
    Vdd  =  model->BSIMvdd;
    if((Ugs  =  here->BSIMugs + here->BSIMugsB * vbs) <= 0 ) {
        Ugs = 0;
        dUgsdVbs = 0.0;
    } else {
        dUgsdVbs  =  here->BSIMugsB;
    }
    if((Uds  =  here->BSIMuds + here->BSIMudsB * vbs + 
            here->BSIMudsD*(vds-Vdd)) <= 0 ) {    
        Uds = 0.0;
        dUdsdVbs = dUdsdVds = 0.0;
    } else {
        Leff  =  here->BSIMl * 1.e6 - model->BSIMdeltaL; /* Leff in um */
        Uds  =  Uds / Leff;
        dUdsdVbs  =  here->BSIMudsB / Leff;
        dUdsdVds  =  here->BSIMudsD / Leff;
    }
    Eta  =  here->BSIMeta + here->BSIMetaB * vbs + here->BSIMetaD * 
        (vds - Vdd);
    if( Eta <= 0 ) {   
        Eta  = 0; 
    ...
    Beta_Vds_0  =  (here->BSIMbetaZero + here->BSIMbetaZeroB * vbs);
    BetaVdd  =  (here->BSIMbetaVdd + here->BSIMbetaVddB * vbs);
    dBetaVdd_dVds  =  MAX( here->BSIMbetaVddD, 0.0); /* Modified */
    if( vds > Vdd ) {
        Beta0  =  BetaVdd + dBetaVdd_dVds * (vds - Vdd);
        dBeta0dVds  =  dBetaVdd_dVds;
        dBeta0dVbs  =  here->BSIMbetaVddB;
    } else {
        VddSquare  =  Vdd * Vdd;
        C1  =  ( -BetaVdd + Beta_Vds_0 + dBetaVdd_dVds * Vdd) / VddSquare;
        C2  =  2 * (BetaVdd - Beta_Vds_0) / Vdd - dBetaVdd_dVds;
        dBeta_Vds_0_dVbs  =  here->BSIMbetaZeroB;
        dBetaVdd_dVbs  =  here->BSIMbetaVddB;
...
SubthresholdComputation:
    ...
    N0  =  here->BSIMsubthSlope;
    Vcut  =  - 40. * N0 * CONSTvt0 ;
    if( (N0 >=  200) || (Vgs_Vth < Vcut ) || (Vgs_Vth > (-0.5*Vcut))) {
        goto ChargeComputation;
    }
    
    NB  =  here->BSIMsubthSlopeB;
    ND  =  here->BSIMsubthSlopeD;
    ...
    Warg2  =  6.04965 * Vtsquare * here->BSIMbetaZero;
    Ilimit  =  4.5 * Vtsquare * here->BSIMbetaZero;
    Iexp = Warg2 * Wgs * Wds;
...
ChargeComputation:
    ...
    if( model->BSIMchannelChargePartitionFlag >= 1 ) {
