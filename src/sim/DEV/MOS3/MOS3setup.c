...
RCSID("MOS3setup.c $Revision: 1.1 $ on $Date: 90/10/11 12:45:40 $")
        ...
        for (here = model->MOS3instances; here != NULL ;
            ...
            *states += MOS3NUMSTATES;
            ...
            if((model->MOS3drainResistance != 0 ||
                ...
                nametemp = (char *)
                        MALLOC((strlen(here->MOS3name)+17)*sizeof(char));
                if(nametemp==(char *)NULL) return(E_NOMEM);
                (void)sprintf(nametemp,"%s#internal#drain",here->MOS3name);
                error = CKTmkVolt(ckt,&tmp,nametemp,(char *)NULL);
            ...
            if((model->MOS3sourceResistance != 0 ||
                ...
                nametemp = (char *)
                        MALLOC((strlen(here->MOS3name)+18)*sizeof(char));
                if(nametemp==(char *)NULL) return(E_NOMEM);
                (void)sprintf(nametemp,"%s#internal#source",here->MOS3name);
                error = CKTmkVolt(ckt,&tmp,nametemp,(char *)NULL);
                if(error) return(error);
